...
class EventsService {
  ...
  Future<List<Map<String, dynamic>>> fetchEvents({bool onlyPublished = true}) async {
    // Construire la requête, filtres d'abord, puis order()
  ...
  Future<Map<String, dynamic>> createOrder({
    ...
    required List<Map<String, dynamic>> items, // {ticket_type_id, quantity, unit_cents}
    String devise = 'EUR',
  }) async {
    ...
    // 1) Créer la commande (MOCK: status=paid ; en prod -> pending + webhook)
    final dynamic ordersResp = await _sb.from('orders').insert({
      'buyer_id': uid,
    ...
    if (orders.isEmpty) {
      throw StateError('Création de commande échouée');
    }
    final Map<String, dynamic> order = Map<String, dynamic>.from(orders.first as Map);
  ...
  Future<Map<String, dynamic>> scanTicket(String qrCode) async {
    ...
    return {'ok': false, 'reason': 'UNEXPECTED_RESPONSE', 'data': resp.toString()};
  }
}
