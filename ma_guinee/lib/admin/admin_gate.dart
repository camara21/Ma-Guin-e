...
/// Barrière d'accès pour les pages Admin — anti-flash:
/// - Pendant le chargement: pas de navigation, simple loader.
/// - Si non connecté: écran avec bouton "Se connecter" (aucune redirection auto).
/// - Si connecté:
///     - rôle admin/owner → rend `child`
///     - sinon → écran "Accès refusé" (option bouton accueil).
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<_GateResult>(
      ...
      builder: (context, snap) {
        ...
        if (snap.hasError) {
          return Scaffold(
            appBar: AppBar(title: const Text('Vérification administrateur')),
            body: Center(
              child: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  const Text('Erreur pendant la vérification des permissions.'),
            ...
            body: Center(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 12),
                  FilledButton(
                    onPressed: () => _goLogin(context), // ← replacement
        ...
        if (res.isAdmin) {
          // Admin confirmé → on rend la page enfant, aucune navigation
          return widget.child;
        }
