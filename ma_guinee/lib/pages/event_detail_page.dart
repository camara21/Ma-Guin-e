...
class _EventDetailPageState extends State<EventDetailPage> {
  ...
  Future<void> _buyTickets() async {
    ...
    if (items.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Sélectionnez au moins 1 billet')));
      return;
    }
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Achat confirmé !')));
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(ev['titre'] ?? 'Événement')),
      body: _loading
                ...
                Expanded(
                  child: ListView(
                    children: _ticketTypes.map((t) {
                      return ListTile(
                        title: Text('${t['nom']} - ${(t['prix_cents'] / 100).toStringAsFixed(2)} ${t['devise']}'),
