...
class ReservationsRestaurantsPage extends StatefulWidget {
  ...
  @override
  State<ReservationsRestaurantsPage> createState() =>
      _ReservationsRestaurantsPageState();
}

class _ReservationsRestaurantsPageState
    extends State<ReservationsRestaurantsPage> {
    ...
    try {
      ...
      if (_scope == 'annulees') {
        ...
        q = q.neq('status', 'annule').or(
              'res_date.lt.$cutDateStr,and(res_date.eq.$cutDateStr,res_time.lt.$cutTimeStr)',
            );
      } else {
        q = q.neq('status', 'annule').or(
              'res_date.gt.$cutDateStr,and(res_date.eq.$cutDateStr,res_time.gte.$cutTimeStr)',
            );
      }
  ...
  void _openLieu(Map<String, dynamic> r) {
    ...
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => RestoDetailPage(restoId: id),
      ),
    );
  }
  ...
  // ============================================================
  // FIX OVERFLOW ICI 100% PROPRE
  // ============================================================
  Widget _buildScopeBar(BuildContext context) {
    final segmented = Theme(
      data: Theme.of(context).copyWith(
        colorScheme:
            Theme.of(context).colorScheme.copyWith(primary: kRestoPrimary),
      ),
    ...
    return Padding(
      ...
      child: LayoutBuilder(
        builder: (context, constraints) {
          // Petits écrans → éviter overflow rouge
      ...
      body: Column(
        children: [
          ...
          Expanded(
            child: _loading
                ? const Center(
                    child: CircularProgressIndicator(color: kRestoPrimary),
                  )
                : RefreshIndicator(
                    color: kRestoPrimary,
                    onRefresh: _load,
                        ...
                        : _items.isEmpty
                            ...
                            : ListView.separated(
                                ...
                                itemBuilder: (_, i) {
                                  final r = _items[i];
                                  final titre =
                                      (r['restaurant_nom'] ?? 'Restaurant')
                                          .toString();
                                  final ville =
                                      (r['restaurant_ville'] ?? '').toString();
